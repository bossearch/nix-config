{
  ...
  home.file.".config/mpv/scripts/uosc-video-settings.lua" = lib.mkIf homes.mpv {
    text = ''
      local mp = require("mp")
      ...
      mp.options.read_options(script_options, "uosc-video-settings", function() end)
      ...
      local function create_property_selection(title, property, selection_options, off_or_default_option, include_custom_item)
      	local property_items = {}
      	local option_match = false

      	for _, item in ipairs(selection_options) do
      		...
      		table.insert(property_items, {
      			...
      			value = is_active and off_or_default_option and command(
      				"adjust-property " .. property .. " " .. off_or_default_option
      			) or command("adjust-property " .. property .. " " .. item.value),
      		})
      	end
      ...
      local function create_aspect_profiles()
      	for aspect_profile in script_options.aspect_profiles:gmatch("([^,]+)") do
      ...
      local function create_color_profiles()
      	if script_options.include_default_color_profile then
      		table.insert(profile.color, {
      			title = script_options.default_color_profile_name:match("^%s*(.-)%s*$"),
      			active = false,
      			value = command(
      	...
      	for color_profile in script_options.color_profiles:gmatch("([^;]+)") do
      ...
      local function create_color_menu()
      	...
      	for _, prop in ipairs(color_properties) do
      		table.insert(
      			color_items,
      			create_property_number_adjustment(
      				prop:gsub("^%l", string.upper),
      				prop,
      				script_options[prop .. "_increment"]
      			)
      		)
      	end
      ...
      local function create_deband_profiles()
      	if script_options.include_default_deband_profile then
      		table.insert(profile.deband, {
      			title = script_options.default_deband_profile_name:match("^%s*(.-)%s*$"),
      			active = false,
      			value = command(
      				"adjust-deband profile "
      	...
      	for deband_profile in script_options.deband_profiles:gmatch("([^;]+)") do
      		...
      		if name and settings then
      			...
      			if iterations and threshold and range and grain then
      				table.insert(profile.deband, {
      					...
      					active = false,
      					value = command(
      						"adjust-deband profile " .. iterations .. "," .. threshold .. "," .. range .. "," .. grain
      					),
      ...
      local function create_deband_menu()
      	...
      	for _, item in ipairs(profile.deband) do
      		if item.id == "profile" then
      			local is_active = item.iterations == iterations
      				and item.threshold == threshold
      				and item.range == range
      				and item.grain == grain
      ...
      local function create_interpolation_menu()
      	...
      	table.insert(
      		interpolation_items,
      		create_property_number_adjustment("Tscale antiring", "tscale-antiring", 0.005, 0, 1)
      	)
      	table.insert(interpolation_items, create_property_number_adjustment("Tscale blur", "tscale-blur", 0.005, 0))
      	table.insert(interpolation_items, create_property_number_adjustment("Tscale clamp", "tscale-clamp", 0.005, 0, 1))
      	table.insert(
      		interpolation_items,
      		create_property_number_adjustment("Tscale radius", "tscale-radius", 0.005, 0.5, 16)
      	)
      	table.insert(interpolation_items, create_property_number_adjustment("Tscale taper", "tscale-taper", 0.005, 0, 1))
      ...
      local function create_scale_menu()
      	...
      	local function add_scale_selection(title, property)
      		...
      		local fixed_scale = {
      			...
      			{ name = "Bicubic Fast", value = "bicubic_fast" },
      			{ name = "Oversample", value = "oversample" },
      		}

      		local non_polar_filter = {
      			{ name = "Spline16", value = "spline16" },
      			{ name = "Spline36", value = "spline36" },
      			{ name = "Spline64", value = "spline64" },
      			{ name = "Sinc", value = "sinc" },
      			{ name = "Lanczos", value = "lanczos" },
      			{ name = "Ginseng", value = "ginseng" },
      			{ name = "Bicubic", value = "bicubic" },
      			{ name = "Hermite", value = "hermite" },
      			{ name = "Catmull Rom", value = "catmull_rom" },
      			{ name = "Mitchell", value = "mitchell" },
      			{ name = "Robidoux", value = "robidoux" },
      			{ name = "Robidoux Sharp", value = "robidouxsharp" },
      			{ name = "Box", value = "box" },
      		...
      		local polar_filter = {
      			{ name = "Jinc", value = "jinc" },
      			{ name = "EWA Lanczos", value = "ewa_lanczos" },
      			{ name = "EWA Hanning", value = "ewa_hanning" },
      			{ name = "EWA Ginseng", value = "ewa_ginseng" },
      			{ name = "EWA Lanczos Sharp", value = "ewa_lanczossharp" },
      			{ name = "EWA Lanczos 4 Sharpest", value = "ewa_lanczos4sharpest" },
      			{ name = "EWA Lanczos Soft", value = "ewa_lanczossoft" },
      			{ name = "Haasnsoft", value = "haasnsoft" },
      			{ name = "EWA Robidoux", value = "ewa_robidoux" },
      			{ name = "EWA Robidoux Sharp", value = "ewa_robidouxsharp" },
      		}
      	...
      	add_scale_selection("Downscale", "dscale")
      ...
      local function create_shader_profiles()
      	if script_options.include_default_shader_profile then
      		table.insert(profile.shader, {
      			title = script_options.default_shader_profile_name:match("^%s*(.-)%s*$"),
      			active = false,
      	...
      	for shader_profile in script_options.shader_profiles:gmatch("([^;]+)") do
      ...
      local function create_shader_menu(value)
      	local shader_items = {}
      	local current_shaders = value or {}

      	-- Helper function to extract just the shader filename for safe matching
      	local function normalize_shader_path(p)
      		if not p then
      			return ""
      		end
      		-- Remove quotes, normalize slashes, and grab the lowercase filename
      		local clean = p:gsub('"', ""):gsub("\\", "/"):lower()
      		local _, filename = mp.utils.split_path(clean)
      		return filename or clean
      	end
      	...
      	-- Robust shader list comparison using normalized file signatures
      	local function compare_shaders(shaders1, shaders2)
      		if #shaders1 ~= #shaders2 then
      			return false
      		end
      		for i, shader in ipairs(shaders1) do
      			if normalize_shader_path(shader) ~= normalize_shader_path(shaders2[i]) then
      				return false
      			end
      		end
      		return true
      	...
      	for _, item in ipairs(profile.shader) do
      		if item.id == "profile" then
      			...
      			for shader in item.profshaders:gmatch("([^,]+)") do
      	...
      	for i, shader_path in ipairs(current_shaders) do
      		-- Index by normalized filename signature so lookups work regardless of path prefix variations
      		local signature = normalize_shader_path(shader_path)
      		is_active[signature] = true
      	...
      	local function listShaderFiles(path, option_path)
      		local dir_items = {}
      		local is_original_path = path == mp.command_native({ "expand-path", script_options.shader_path })
      		...
      		if shader_files ~= nil then
      			...
      			for _, shader_path in ipairs(shader_files) do
      				...
      				-- Check the active state using the exact same signature rule
      				local file_signature = normalize_shader_path(shader_path)

      				table.insert(dir_items, {
      					title = shader:match("(.+)%..+$") or shader,
      					icon = is_active[file_signature] and "check_box" or "check_box_outline_blank",
      	...
      	for _, item in
      		ipairs(
      			listShaderFiles(
      				mp.command_native({ "expand-path", script_options.shader_path }),
      				script_options.shader_path
      			)
      		)
      	do
      		table.insert(shader_items, item)
      ...
      local function setup_property_observers()
      	mp.observe_property("video-aspect-override", "number", function(_, value)
