{
  ...
  home.file.".config/waybar/scripts/utility/lamp-control.sh" = lib.mkIf homes.waybar {
    executable = true;
    text = ''
      #!/usr/bin/env bash

      RECEIVE_PIPE="$HOME/.cache/${hosts.username}/lamp-control"
      LAMP_IP="192.168.18.14"
      PORT=38899
      export GTK_ICON_THEME=${homes.icon}
