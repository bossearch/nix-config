{
  ...
  home.file.".config/waybar/scripts/resource/graph.sh" = lib.mkIf homes.waybar {
    executable = true;
    text = ''
      #!/usr/bin/env bash

      case "$1" in
      cpu) TEMP_FILE="$HOME/.cache/${hosts.username}/cputemp" ;;
      gpu) TEMP_FILE="$HOME/.cache/${hosts.username}/gputemp" ;;
      *) exit 1 ;;
      esac
