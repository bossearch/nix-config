{
  ...
  programs.nixvim = {
    autoCmd =
      ...
      ++ lib.optionals (builtins.elem pkgs.vimPlugins.fyler-nvim config.programs.nixvim.extraPlugins) [
        # auto open fyler
        {
          event = ["BufWinEnter"];
          callback.__raw = ''
      ...
      ++ lib.optionals (config.programs.nixvim.plugins.undotree.enable) [
